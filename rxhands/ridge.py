...
def main(data_folder="./data/", results_folder="./results/"):
    ...
    for fname in os.listdir(data_folder) :
        if fname.endswith(".png") or fname.endswith(".tiff") :
            ...
            ridge_img = skimage.filters.sato(img, sigmas=range(1, 5), black_ridges=False)
            #ridge_black_img = skimage.filters.sato(img, sigmas=range(1, 5), black_ridges=True)
            ridge_img[ridge_img < 0 ] = 0.
            ridge_norm = cv2.normalize(ridge_img, None, 0, 255, cv2.NORM_MINMAX)
            sobelx = cv2.Sobel(src=ridge_norm, ddepth=cv2.CV_64F, dx=0, dy=1, ksize=5)
            sobelx[sobelx < 0 ] = 0.
            ...
            save_img(ridge_img, results_folder + "ridge/" + fname)
